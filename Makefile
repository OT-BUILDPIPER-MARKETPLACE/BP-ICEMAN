...
run-debug:
	docker run -it --rm --name schedule_instance --network host -v ${CONF_PATH}:/opt/config/schedule_resources_backup.yml:ro -e SCHEDULE_ACTION=${ACTION} -e CONF_PATH='/opt/config/schedule_resources_backup.yml' -v ~/.aws:/root/.aws --entrypoint bash opstree/schedule_instance:${VERSION} 
